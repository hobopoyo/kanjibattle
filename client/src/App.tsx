...
const grades: Array<{ value: GradeKey; label: string }> = [
  { value: 'grade1', label: '小学1年生' }, { value: 'grade2', label: '小学2年生' }, { value: 'grade3', label: '小学3年生' },
  { value: 'grade4', label: '小学4年生' }, { value: 'grade5', label: '小学5年生' }, { value: 'grade6', label: '小学6年生' },
  { value: 'juniorHigh', label: '中学生以上' }, { value: 'advanced', label: 'AP Japanese / Advanced Learners' }
];
        ...
        <section className="mx-auto flex min-h-screen max-w-5xl items-center justify-center">
          <div className="grid w-full gap-6 rounded-[2rem] bg-white/90 p-6 shadow-soft md:grid-cols-[1.1fr_0.9fr] md:p-10">
            <div className="flex flex-col justify-center">
              ...
              <p className="mt-5 text-lg font-bold leading-8 text-slate-600">読み方や意味を見て漢字を書き、みんなは四択で答える、教室向けの対戦型漢字ゲームです。</p>
            </div>
      ...
      <main className="min-h-screen bg-gradient-to-br from-yuzu/40 via-white to-sakura/20 p-4 text-sumi">
        <section className="mx-auto max-w-4xl py-10">
          ...
          <div className="mt-8 grid gap-3">
            {sortedPlayers.map((p, index) => <div key={p.id} className="flex items-center justify-between rounded-3xl bg-white p-5 shadow-soft"><div className="text-2xl font-black">{index + 1}位 {p.name}</div><div className="text-xl font-black text-sakura">{p.score}点 / 正解 {p.correctCount}</div></div>)}
          </div>
          ...
          <div className="panel">
            ...
            <fieldset disabled={!view.you?.isHost} className="mt-5 grid gap-4">
              ...
              <select className="input" value={view.settings.mode} onChange={(e) => updateSettings({ mode: e.target.value as GameSettings['mode'] })}>
                <option value="grade">学年別漢字モード</option><option value="custom">教師指定漢字モード</option><option value="review">今日の復習モード</option>
              </select>
    ...
    <main className="min-h-screen bg-gradient-to-br from-sora/25 via-white to-yuzu/30 p-3 text-sumi">
      <section className="mx-auto grid max-w-7xl gap-4 lg:grid-cols-[1fr_330px]">
        <div>
          ...
          {view.isDrawer && <div className="mb-3 rounded-[1.5rem] bg-yuzu/40 p-4 text-center"><p className="text-lg font-black">お題：{view.currentTurn?.prompt}</p><p className="font-bold">この{view.currentTurn?.promptType === 'reading' ? '読み方' : '意味'}の漢字を書いてください。答え：{view.currentTurn?.answer}</p></div>}
          {!view.isDrawer && <div className="mb-3 rounded-[1.5rem] bg-sora/20 p-4 text-center text-lg font-black">手書き漢字を見て、正しい漢字を選ぼう</div>}
          ...
          {view.currentTurn?.answer && !view.isDrawer && view.phase === 'turn-reveal' && <div className="mt-4 rounded-3xl bg-white p-5 text-center text-4xl font-black shadow-soft">正解：{view.currentTurn.answer}</div>}
        </div>
